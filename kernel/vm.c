...
uvmcopy(pagetable_t old, pagetable_t new, uint64 sz)
{
 ...
 int uvmcopy_v2(pagetable_t src, pagetable_t dst, uint64 oldsz, uint64 newsz) {
   pte_t *from;
   pte_t *to;
   oldsz = PGROUNDUP(oldsz);
   for (uint64 i = oldsz; i < newsz; i += PGSIZE) {
     if ((from = walk(src, i, 0)) == 0)
       panic("uvmcopy_v2: walk user pagetable error");
     if ((to = walk(dst, i, 1)) == 0)
       panic("uvmcopy_v2: walk user's kernel pagetable error");
...
void vmprint_dfs(pagetable_t pagetable, int depth) {
  ...
  for (int i = 0; i < 512; i++) {
    ...
    if ((pte & PTE_V)) {
      ...
      printf("%d: pte %p pa %p\n", i,pte, pa);
      vmprint_dfs((pagetable_t)pa, depth+1);
    }
  }
}